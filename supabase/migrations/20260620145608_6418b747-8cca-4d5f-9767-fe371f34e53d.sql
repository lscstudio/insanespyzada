...
begin
  ...
  if exists (select 1 from cron.job where jobname = 'adspy-collect-hourly-retry-1') then
    perform cron.unschedule('adspy-collect-hourly-retry-1');
  end if;
  if exists (select 1 from cron.job where jobname = 'adspy-collect-hourly-retry-2') then
    perform cron.unschedule('adspy-collect-hourly-retry-2');
  end if;
end $$;
